...
- **Concurrent review logs interleaved** — The review subprocess inherited the webhook server's stdout/stderr, causing output from multiple parallel reviews to mix together in the server logs. The subprocess now runs with `stdout=subprocess.PIPE, stderr=subprocess.PIPE` and its output is forwarded line-by-line through the server's structured logger with full `[ENV:…][owner/repo#N]` context tags
- **Silent fallback on unexpected Claude response structure** — `call_claude()` had a bare `except Exception: pass` when extracting text from the Bedrock API response. If the response shape ever changes, the exception was swallowed silently and the raw JSON dump was returned, causing a confusing downstream parse failure. Now logs a `WARNING` to stderr before falling back
