#!/usr/bin/env python3
"""CircleCI runner for HawkEye Reviewer (GHEC and GHES).

Runs inside a CircleCI job.  Reads CircleCI environment variables to
discover the repo and branch, authenticates via the GitHub App, finds
...
Supports both GHEC (api.github.com) and GHES (custom API URL).

Required environment (from a shared CircleCI context):
  HAWKEYE_APP_ID              GitHub App ID
  HAWKEYE_APP_PRIVATE_KEY     GitHub App private key (PEM contents)

  Legacy GHES names also accepted (fallback):
    GHES_APP_ID / GHES_APP_PRIVATE_KEY

Optional environment:
  HAWKEYE_GITHUB_API_URL      GitHub API URL (default: https://api.github.com)
  HAWKEYE_APP_SLUG            App slug (enables "Re-request review" button)
  HAWKEYE_SSL_CA_BUNDLE       CA bundle for TLS (inline PEM or file path)
  SERVER_PRIVATE_KEY          RSA key for decrypting HAWKEYE_CLAUDE_BLOB

  Legacy GHES names also accepted (fallback):
    GITHUB_API_URL / GHES_APP_SLUG / GHES_SSL_CA_BUNDLE
...
def main() -> None:
    ...
    if not app_id or not private_key:
        print("ERROR: HAWKEYE_APP_ID and HAWKEYE_APP_PRIVATE_KEY must be set "
              "(typically via a shared CircleCI context). "
              "Legacy GHES_APP_ID / GHES_APP_PRIVATE_KEY are also accepted.",
              file=sys.stderr)
        sys.exit(1)
